...
// ===== Tier Verification (shared single source of truth) =====
const { VERSION_ACCESS, TIER_ORDER, TIERS } = require('./tiers');

// Bug 1 fix: Check if user's tier access >= requested version
function canAccessVersion(tier, version) { return (VERSION_ACCESS[tier] || 0) >= version; }
function getTierForVersion(version) {
  for (const [tier, minVer] of Object.entries(VERSION_ACCESS)) { if (minVer === version) return tier; }
  return 'free';
}
...
async function securityCheck(req, version) {
  ...
  // 5. Session token — REQUIRED for ALL versions (including v1, v5, v6)
  ...
  // Fix #7: ALL versions require authentication
  if (!sessionToken) {
    ...
    // Verify binding signature freshness (5 minutes)
    const ts = parseInt(bindingTimestamp);
    if (isNaN(ts) || Math.abs(Date.now() - ts) > 5 * 60 * 1000) {
      auditLog('BINDING_EXPIRED', { deviceId, userEmail, version, ip: clientIp });
  ...
  // 8. HMAC signature — REQUIRED for ALL versions
  ...
  // 9. Tier access (all versions require tier check now)
  ...
  // 10. Server-side quota check (Redis, NOT client-reported)
  const { enforceQuota } = require('./quota');
  const quotaError = await enforceQuota(apiKey, userTier);
  if (quotaError) {
    auditLog('QUOTA_EXCEEDED', { userEmail, userTier, ip: clientIp });
    result.error = quotaError.message; result.status = 429; return result;
  }
  ...
  result.estInputTokens = Math.ceil((bodyStr || '').length / 4);
  return result;
}
