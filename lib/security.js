...
function canAccessVersion(tier, version) { return version >= (VERSION_ACCESS[tier] || 1); }
...
async function securityCheck(req, version) {
  ...
  // 5. Session token (v2+ mandatory, v5 skip for free ZenMux)
  ...
  // 9. Tier access (v5 = free ZenMux, skip tier check)
  if (version !== 5 && !canAccessVersion(userTier, version)) {
  ...
  // 10. Credit check (token-based: estimate input tokens, deduct actual after response)
  const credits = await getCredits(apiKey);
  const estInputTokens = Math.ceil((bodyStr || '').length / 4);
  if (credits && credits.remaining < Math.max(estInputTokens, 10)) {
    auditLog('CREDIT_EXCEEDED', { userEmail, estInputTokens, remaining: credits.remaining });
    result.error = `Insufficient credits. Estimated need ~${estInputTokens}, have ${credits.remaining}`; result.status = 429; return result;
  }
  ...
  result.estInputTokens = estInputTokens;
  return result;
}
