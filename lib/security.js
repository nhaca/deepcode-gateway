...
// ===== Gateway credentials =====
// Priority: env vars > hardcoded fallbacks (remove fallbacks after setting env vars)
const GATEWAY_KEY = process.env.GATEWAY_KEY || 'dc-1b4fd9892c8fecd9a103a49e0b52ab3e2d985d10';
const GATEWAY_SECRET = process.env.GATEWAY_SECRET || 'dc-c91d93094cd58202a0230fab112432f7833111f7';

if (!process.env.GATEWAY_KEY || !process.env.GATEWAY_SECRET) {
  console.warn('SECURITY: Using hardcoded fallback keys. Set GATEWAY_KEY and GATEWAY_SECRET env vars on Vercel!');
}
...
function securityCheck(req, version) {
  ...
  // 1. Client ID (auto-generated per IDE instance)
  const clientId = req.headers['x-client-id'];
  if (!clientId || !clientId.startsWith('dc-')) {
    auditLog('CLIENT_ID_MISSING', { ip: getClientIp(req) });
    result.error = 'Missing or invalid client ID'; result.status = 401; return result;
  }
  ...
  if (!isDeviceAllowed(deviceId)) {
    auditLog('DEVICE_NOT_ALLOWED', { deviceId, clientId, ip: clientIp });
    result.error = 'Device not registered'; result.status = 403; return result;
  }

  // 4. User identity (optional for v1 free tier — anonymous allowed)
  ...
  if (version >= 2) {
    ...
    if (!bindingSig || !bindingTimestamp || !loginIp) {
      auditLog('BINDING_MISSING', { deviceId, clientId, userEmail, version, ip: clientIp });
    ...
    if (isNaN(bindingAge) || bindingAge > BINDING_MAX_AGE_MS) {
      auditLog('BINDING_EXPIRED', { deviceId, clientId, userEmail });
    ...
    const validBinding = verifyBindingSignature(bindingSig, deviceId, userEmail, userProvider || '', loginIp || '', bindingTimestamp);
    if (!validBinding) {
      auditLog('BINDING_FORGERY_V1', { deviceId, clientId, userEmail });
      result.error = 'Invalid binding signature'; result.status = 403; return result;
    }
  }
  ...
  // 7. Rate limit (by client ID + device ID)
  if (!checkRateLimit(`${clientId}:${deviceId}`)) {
  ...
  if (!verifySignature(signature, timestamp, bodyStr, deviceId)) {
    ...
    auditLog('SIGNATURE_INVALID', { deviceId, clientId, ip: clientIp });
  ...
  if (!canAccessVersion(userTier, version)) {
    auditLog('TIER_BYPASS_ATTEMPT', { deviceId, clientId, userEmail, userTier, requestedVersion: version });
