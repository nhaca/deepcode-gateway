...
// ===== IP Consistency (IPv4 + IPv6 support) =====
...
// ===== Rate Limiter (use hash of full API key to avoid collision) =====
...
async function securityCheck(req, version) {
  ...
  if (isIpBlacklisted(clientIp)) { result.error = 'IP temporarily blocked due to repeated failures'; result.status = 429; return result; }
  ...
  // 7. Rate limit (using full key hash)
