...
async function generateApiKey(email, tier = 'free', provider = 'unknown') {
  ...
  if (redisResult !== null) {
    ...
    await redisCommand('SET', `credits:${key}`, JSON.stringify({ remaining: limit, limit, resetAt: getNextMonthTimestamp() }));
  } else {
...
async function getCredits(apiKey) {
  ...
  if (!raw) return { remaining: 100, limit: 100, resetAt: getNextMonthTimestamp() };
