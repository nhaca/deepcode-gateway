...
// ===== Tier Credit Limits (shared single source of truth) =====
const { CREDITS: TIER_CREDIT_LIMITS } = require('../../shared/tiers');
...
async function getCredits(apiKey) {
  if (!isRedisAvailable()) {
    // Fail-closed: no Redis = no credits = reject requests
    return { remaining: 0, limit: 0, resetAt: 0, redisAvailable: false };
  }
